...
long down(char param1[], char param2[]) {   
    ...
    //if ( count != 3 ){
      //  printf("usage: %s <url> <resource>\n", argv[0]);
        //exit(0);
    //}
    char *dest_url = param1; //"https://monitor.uac.bj:4449";
    char *dest = param2;
 //   printf("DESt: %s", dest);
...
//    req.path = (char*) malloc(strlen(param2)*sizeof(char));
   // printf("path 1: %s", req.path);  
    //strncpy(req.path,chemin, strlen(chemin));  
   // printf("path 2: %s", req.path);  
  //  req.path = (char*)strcat(req.path, dest); 
   //  printf("path init1: %s", req.path); 
